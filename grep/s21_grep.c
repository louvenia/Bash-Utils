...
int main(int argc, char *argv[]) {
  ...
  FLAGS fg = {};
  if (argc > 2) {
    error_n = oppars(&fg, argc, argv, &patterns, &array_p, &f_patterns, &array_f);
    if (error_n == 0) {
      if (fg.f) error_n = get_fpatterns(&patterns, &array_p, f_patterns, array_f);
...
int reader(FLAGS fg, int argc, char *argv[], char **patterns, int array_p) {
  ...
  for (int i = 1; i < argc; i++) {
    if (argv[i][0] != '-') {
      ...
      if (file == NULL) {
        error_n = 2;
        if (fg.s == 0) printf("s21_grep: %s: No such file or directory\n", argv[i]);
      } else {
        char str[BUFF_S + 1] = "";
...
void process_pcre(FLAGS fg, char *str, long unsigned *line_n, char **patterns, int array_p,
                     char *f_name, long unsigned *line_nOUT) {
  (*line_n)++;
  char l_number[25] = "";
  int options = 0;
  int patterns_v = 0;
  ...
  for (int j = 0; j < array_p; j++) {
    ...
    if (re == NULL) {
      ...
      if (fg.o && fg.v == 0 && fg.c == 0 && fg.l == 0 && rc > 0) {
        ...
        while (offset < length &&
               (rc = pcre_exec(re, 0, str, length, offset, 0, ovector, sizeof(ovector))) >= 0) {
          for (int i = 0; i < rc; ++i) {
            if (line_p == 0) {
              printf("%s%s%.*s\n", f_name, l_number, ovector[2 * i + 1] - ovector[2 * i],
                     str + ovector[2 * i]);
              line_p = 1;
            } else {
...
int get_fpatterns(char ***patterns, int *array_p, char **f_patterns, int array_f) {
  int error_n = 0;
  FILE *file = NULL;
